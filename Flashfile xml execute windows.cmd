...
for /F delims^=^"^ tokens^=4^,6^,8 %%a in (flash.txt) do echo @if not exist %%a echo %%a not exist^&set notexistfiles=1 >> flashfile.cmd
...
findstr /v "@title" flashfile.cmd > "flashfile %title%.txt"
findstr /v "@if" "flashfile %title%.txt" > "FLASHFILE COMMANDS %title%.txt"
del "flashfile %title%.txt"
